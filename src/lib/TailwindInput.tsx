...
const TailwindClassInput = ({ value = '', onChange, placeholder = 'Search Tailwind classes...' }) => {
  ...
  const filteredOptions = useMemo(() => {
    ...
    return allClasses
      .filter((cls) => cls.toLowerCase().includes(query) && !selectedClasses.includes(cls))
      .slice(0, 8)
      .map((cls) => ({
        value: cls,
        label: (
          <div className="flex items-center justify-between py-1">
            ...
            <span className="text-xs px-2 py-0.5 bg-blue-600 text-white rounded">{getCategoryForClass(cls)}</span>
          </div>
        ),
      }));
