...
const ResponsiveGridLayout = () => {
  ...
  const handleDrag = (elementId, { left, top }) => {
    ...
    const hasCollision = elements.filter((el) => el.id !== elementId).some((other) => checkCollision(testElement, other));

    if (!hasCollision) {
      setElements((prevElements) => {
        const newElements = prevElements.map((el) =>
          el.id === elementId ? { ...el, x: constrainedX, y: constrainedY } : el
        );
  ...
  const handleResize = (elementId, { width, height, drag }) => {
    ...
    if (!hasCollision) {
      setElements((prevElements) =>
        prevElements.map((el) => (el.id === elementId ? { ...el, w: constrainedW, h: gridH } : el))
      );
    }
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <h1 className="text-2xl font-bold text-gray-800">Responsive Grid Layout</h1>
      ...
      <div
        ...
        style={{
          ...
          `,
          backgroundSize: `${100 / gridConfig[breakpoint].cols}% ${
            gridConfig[breakpoint].rowHeight + gridConfig[breakpoint].margin[1]
          }px`,
        }}
      ...
      >
        {elements.map((element) => (
          <div
            ...
            className={`bg-gradient-to-br ${element.color} rounded-lg shadow-lg cursor-pointer select-none overflow-hidden ${
              selectedElement === element.id ? 'ring-2 ring-blue-400 ring-offset-2' : ''
            }`}
          ...
          >
            <div className="p-4 h-full flex flex-col">
              <div className="text-white font-semibold text-sm mb-2">{element.content}</div>
