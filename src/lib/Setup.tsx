...
import { logDragStart, logDragEnd, logStateUpdate, logSortableActive, logSortableInactive } from './dragDropDebugger';
          ...
          const sortableInstance = Sortable.create(container as HTMLElement, {
            ...
            onStart: (evt) => {
              try {
                ...
                logSortableActive();
                logDragStart(elementId || 'unknown', draggedElement?.parentElement?.id);
              } catch (error) {
            ...
            onEnd: (evt) => {
              ...
              try {
                ...
                logDragEnd(movedId, newParentId || 'root', oldParentId || 'root');
                ...
                setTimeout(() => {
                  try {
                    logStateUpdate({ action: 'updating_state', movedId, newParentId, oldParentId });
                    if (setElements && typeof setElements === 'function') {
                      setElements?.((currentElements) => {
                        try {
                          ...
                          logStateUpdate({ action: 'state_updated_successfully', elementCount: updatedElements.length });
                      ...
                      console.warn('setElements function not available');
                    }
