...
const FigmaTailwindEditor = ({ value = '', onChange }) => {
  ...
  const assignControlledClass = (cls, props, state = 'base') => {
    ...
    // Flexbox & Grid
      ...
      /^(justify-normal|justify-start|justify-end|justify-center|justify-between|justify-around|justify-evenly|justify-stretch)$/.test(
        cls
      )
    ) {
  ...
  const generateClassName = () => {
    ...
    return [...preservedClassesList, ...controlledClasses, ...stateClasses].filter((cls) => cls.trim()).join(' ');
  };
  ...
  const Section = ({ title, icon: Icon, children, sectionKey }) => (
    <div className="border-b border-[#323232]">
      ...
      {!collapsedSections[sectionKey] && <div className="p-3">{children}</div>}
    </div>
  );
      ...
      <Section title="Custom Classes" icon={Plus} sectionKey="custom">
        <div className="grid grid-cols-2 gap-2">
          <div className="flex gap-1">
            <Input value={customClass} placeholder="Add custom class..." onChange={setCustomClass} />
            <button
